...
import { ITodo, IUpdateTodoInput, IDatabaseTodo, IDatabase, TodoError } from '../types';
...
class Database implements IDatabase {
    ...
    public getAllTodos(): Promise<ITodo[]> {
        return new Promise((resolve, reject) => {
            ...
            this.db.all(sql, [], (err: Error | null, rows: IDatabaseTodo[]) => {
                if (err) {
                    reject(new TodoError(`获取todos失败: ${err.message}`, 'DB_QUERY_ERROR', 500));
                } else {
                    // 转换数据格式以匹配前端
    ...
    public createTodo(text: string): Promise<ITodo> {
        return new Promise((resolve, reject) => {
            ...
            this.db.run(sql, [text.trim(), false], (err: Error | null) => {
                if (err) {
                    reject(new TodoError(`创建todo失败: ${err.message}`, 'DB_INSERT_ERROR', 500));
                } else {
                    // 返回新创建的TODO项
                    const getTodoSql = 'SELECT * FROM todos WHERE id = last_insert_rowid()';
                    if (!this.db) {
                        reject(new TodoError('数据库连接丢失', 'DB_CONNECTION_LOST', 500));
                        return;
                    }
                    
                    this.db.get(getTodoSql, [], (err: Error | null, row: IDatabaseTodo) => {
                        if (err) {
                            reject(new TodoError(`获取新创建的todo失败: ${err.message}`, 'DB_QUERY_ERROR', 500));
                        } else if (!row) {
                            reject(new TodoError('新创建的todo不存在', 'TODO_NOT_FOUND', 404));
                        } else {
                            const todo: ITodo = {
                                id: row.id,
                                text: row.text,
    ...
    public updateTodo(id: number, updates: IUpdateTodoInput): Promise<boolean> {
        return new Promise((resolve, reject) => {
            ...
            this.db.run(sql, values, function(this: sqlite3.RunResult, err: Error | null) {
                if (err) {
                    reject(new TodoError(`更新todo失败: ${err.message}`, 'DB_UPDATE_ERROR', 500));
                } else {
    ...
    public deleteTodo(id: number): Promise<boolean> {
        return new Promise((resolve, reject) => {
            ...
            this.db.run(sql, [id], function(this: sqlite3.RunResult, err: Error | null) {
                if (err) {
                    reject(new TodoError(`删除todo失败: ${err.message}`, 'DB_DELETE_ERROR', 500));
                } else {
                    resolve(this.changes > 0);
                }
            });
    ...
    public deleteCompletedTodos(): Promise<number> {
        return new Promise((resolve, reject) => {
            ...
            this.db.run(sql, [], function(this: sqlite3.RunResult, err: Error | null) {
                if (err) {
                    reject(new TodoError(`删除已完成todos失败: ${err.message}`, 'DB_DELETE_ERROR', 500));
                } else {
